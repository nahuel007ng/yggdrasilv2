...
async def get_recommendation(payload) -> dict:
    ...
    if topic == "books":
        ...
        if not data.get("books_finished"):
            return {
                "success": True,
                "reply": "Gran Maestro, todavía no registraste pergaminos terminados, así que no tengo de dónde sacar tus gustos. Contame cuando termines uno ('terminé Hamlet, es un clásico') y te recomiendo el siguiente.",
            }
    ...
    if topic == "finance":
        ...
        if not data.get("by_category"):
            return {
                "success": True,
                "reply": "Gran Maestro, todavía no hay suficientes gastos registrados para analizar dónde recortar.",
            }
        reply = await compose_reply(question, "finance_advice", data)
        return {"success": True, "reply": reply}

    return {"success": False, "error": f"Gran Maestro, aún no sé recomendar sobre '{topic}'. Puedo con libros y finanzas."}
